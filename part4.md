...
### 4.1.3 The Application Class (Hub)
...
### 4.1.5 Sharding in CI (from the wiki)
...
### 4.2.2 A Test With Speculos (Device Interaction)
...
### 4.2.4 Template: Write Your Own Test
...
### 4.2.5 The 4-Step Development Workflow
...
### 4.3.5 Auto-Waiting — Playwright's Killer Feature
...
### 4.3.6 Configuration — `playwright.config.ts` Line by Line
...
### 4.3.7 Running Tests from the CLI
...
### 4.3.8 Visual Debugging with PWDEBUG
...
### 4.4.2 `base.extend<T>()` — Creating Custom Fixtures
...
### 4.4.3 The Ledger Live `TestFixtures` Type
...
### 4.4.4 Overriding Fixtures with `test.use()`
...
### 4.4.5 Page Object Model (POM) — The Architecture
...
### 4.4.6 TypeScript Decorators — The `@step()` Pattern
...
### 4.4.8 Parallel Execution and Workers
...
### 4.5.2 How Playwright Launches the Electron App
...
### 4.5.3 The Environment Variables
...
### 4.5.4 `ElectronApplication` vs `Page`
...
### 4.5.5 The Application Startup Sequence
...
| File | Purpose |
|------|---------|
| `playwright.config.ts` | Test runner configuration (covered in Ch 4.3.6) |
...
### 4.6.6 Modal, Drawer, and Dialog Objects
...
### 4.6.9 Test Data Enums (from `@ledgerhq/live-common`)
...
### 4.6.10 The Modular Dialog System
...
### 4.7.1 Step 1: Read the Jira Ticket
...
### 4.7.2 Step 2: Check What Already Exists
...
### 4.7.3 Step 3: Set Up Your Environment
...
### 4.7.4 Step 4: Explore the App Manually
...
### 4.7.5 Step 5: Plan Your Test Structure
...
# Run with debug mode (step-by-step) — see Chapter 4.3.8
...
### 4.7.9 Step 9: Check the Allure Report
...
### 4.7.10 Step 10: Commit, Push, Create PR
...
# Commit with conventional commit format (see Chapter 7.3)
...
This command will ask for your ticket URL, description, change type, scope, and test coverage. It creates a changeset, generates a structured PR body, pushes, creates a **draft PR**, and opens it in your browser. See Chapter 4.8.9 for a full walkthrough.
...
### 4.8.2 Analyzing Existing Coverage
...
### 4.8.4 Run the Test in Isolation
...
> **Tip:** Use Playwright's debug mode to step through the test visually — see Chapter 4.3.8 for details:
> ```bash
...
### 4.8.9 Create the PR with Claude Code
...
### 4.8.10 Mark the PR as Ready and Merge
...
### 4.8.11 Reference: Writing a New Test from Scratch
...
<div class="chapter-intro">
This is your second real ticket walkthrough. Where Chapter 4.8 walked through a simple <strong>Add Account</strong> ticket, this one covers a different feature area — the <strong>Market page</strong> — and introduces new concepts: <strong>Wallet 4.0 feature flags</strong>, <strong>cross-platform coverage gaps</strong>, and <strong>the filter dropdown pattern</strong>. The workflow is the same; the context is new.
</div>

### 4.9.1 Understanding the Ticket
...
### 4.9.2 Analyzing Existing Coverage
...
### 4.9.3 Understanding the Desktop Market Architecture

Before implementing, let us understand the key architectural elements this test uses. These are different from the Add Account flow in Chapter 4.8.
...
Unlike the Add Account test (Chapter 4.8) which used `skip-onboarding-with-last-seen-device` (empty portfolio), the market test uses `speculos-tests-app`. This userdata file contains:
...
### 4.9.5 Run the Test in Isolation
...
### 4.9.9 Mark Ready, Merge, and Update Xray
...
### 4.9.10 Reference: Writing a New Market Filter Test from Scratch
...
### 4.9.11 Key Concepts Summary

This walkthrough introduced several concepts not covered in Chapter 4.8:
...
### 4.10.1 Exercise 1: Trace the Fixture Lifecycle (30 min)
...
### 4.10.2 Exercise 2: Add an Assertion to a Page Object (30 min)
...
### 4.10.3 Exercise 3: Read and Explain a Test (45 min)
...
### 4.10.4 Exercise 4: Debug a Broken Test (45 min)
...
**Verification:** Fix all 5 bugs and compare against the template in Chapter 4.7.6.

### 4.10.5 Exercise 5: Write a Test from Scratch (60 min)
...
### 4.10.6 Exercise 6: Extend the Add Account Test (45 min)

**Objective:** Add new assertions to an existing test.

**Instructions:**
1. After successfully understanding the add account test (Ch 4.8), add these verifications:
...
### 4.10.7 Challenge: Write a Complete Send BTC Test (90 min)
...
- The daily workflow checklist (Ch 4.7)
...
Part 4 took you from the desktop suite's architecture (PageHolder, Component, AppPage, Application) through Playwright core, fixtures, Electron specifics, Speculos integration, the codebase file-by-file, the daily workflow, two real ticket walkthroughs, and hands-on exercises. This assessment tests the load-bearing ideas you need in daily work. You must score 80% to pass. Take your time, revisit any chapter you stumble on, then move on to Part 5.
...
<p class="quiz-explanation">Ch 4.3 §22.5 describes auto-waiting as Playwright's "killer feature": five actionability checks retried until timeout. This is why you rarely need manual <code>sleep()</code> calls in well-written specs.</p>
</div>
...
<p class="quiz-explanation">Ch 4.3 §22.2 ranks <code>getByTestId()</code> first when a test ID is available. Text and role locators are still useful, but they break when copy or ARIA attributes change. CSS selectors are a last resort.</p>
</div>
...
<p class="quiz-explanation">Ch 4.4 §23.2 describes the <code>use()</code> callback pattern explicitly: setup → <code>await use(resource)</code> → teardown. Teardown runs regardless of test outcome, which is why fixtures are the correct place for resource cleanup.</p>
</div>
...
<p class="quiz-explanation">Ch 4.5 §24.4 contrasts the two: <code>electronApp</code> is for app-level operations and window enumeration, <code>page</code> is the main window. Page objects that touch webviews (Swap, Buy/Sell) need both, because the webview is reached via <code>electronApp.windows()</code>.</p>
</div>
...
<p class="quiz-explanation">Ch 4.5 §24.6 shows the pattern: <code>const [, webview] = electronApp.windows();</code> then interact with <code>webview.getByTestId(...)</code>. Webviews are separate windows, not frames. The <code>WebviewLogCollector</code> collects console/network logs from all webview windows for failure triage.</p>
</div>
...
<p class="quiz-explanation">Ch 4.9 §29.2-29.5 showed that the existing <code>add.account.spec.ts</code> BTC entry already covered "first time add account" through <code>userdata: "skip-onboarding-with-last-seen-device"</code>. The fix was a single-line metadata change so Xray could map the test case to the automation. Framework helper <code>addTmsLink()</code> splits the comma-separated <code>xrayTicket</code> string and registers each ID with Allure.</p>
</div>
