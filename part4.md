...
### 4.1.3 The WebSocket Bridge In Depth
...
### 4.1.4 Why `app.init()` is in `beforeAll` (not `beforeEach`)
...
### 4.1.5 Key Differences from Desktop (Summary)
...
### 4.2.2 Mobile Test With Speculos
...
### 4.2.3 Desktop to Mobile Translation
...
### 4.3.3 The `testID` Prop — Your Best Friend
...
### 4.3.4 Platform Module & Cross-Platform Branching
...
### 4.3.5 Metro — The JavaScript Bundler
...
### 4.3.6 Deep Links & the `Linking` API
...
### 4.4.2 macOS-Only Reality (for iOS)
...
### 4.4.3 Version Management — `proto` and `mise`
...
### 4.4.4 iOS Side — Xcode to CocoaPods
...
### 4.4.5 Android Side — Studio to Gradle to adb
...
### 4.4.9 Verifying Your Setup — A Checklist
...
### 4.5.2 The Matcher–Action–Expectation Triad
...
### 4.5.6 `element()` Is Not an Element
...
### 4.5.7 `waitFor(...)` — Polling With a Timeout
...
### 4.5.9 Your First Detox Test — Line by Line
...
### 4.5.10 The Platform Split — EarlGrey vs Espresso
...
<div class="chapter-intro">
Chapter 4.5 taught you the grammar. This chapter teaches you the machinery: how Detox is configured, how it stays in sync with the app, how Jest drives it, and — critically — how Ledger Live's custom WebSocket bridge injects Redux state, feature flags, and mock device events into the running mobile app. By the end you will be able to read every line of <code>detox.config.js</code>, <code>jest.config.js</code>, and the bridge server/client, and you will know when to reach for <code>device.disableSynchronization()</code> (spoiler: almost never).
</div>

### 4.6.1 `detox.config.js` — The Top-Level Map
...
### 4.6.3 How Detox Synchronizes — Idle Detection
...
### 4.6.4 Synchronization Troubles — When to Give Up
...
### 4.6.5 Jest as the Runner — `jest.config.js`
...
### 4.6.6 `jest.globalSetup.ts` and `jest.environment.ts`
...
### 4.6.7 The WebSocket Bridge — Why Detox Alone Isn't Enough
...
### 4.6.8 `@Step` — Allure Step Decorators on POM Methods
...
### 4.6.9 `$TmsLink("B2CQA-XXX")` — Jira/Xray Linkage
...
- CocoaPods synced (Chapter 4.4 `pod install` in `apps/ledger-live-mobile/ios`).
...
### 4.8.9 Running a Single `it()` by TmsLink
...
Chapter 3.5 (desktop) and Chapter 4.6 (mobile) both introduced Speculos. Mobile E2E has two modes:
...
- `$0`, `$1`, … are substituted with the method's arguments at call time — same semantics as Chapter 2.5.2's desktop `@step("Click on asset $0")`.
- If the method throws, the step is marked `failed` and the screenshot is attached automatically by the global Jest retry handler (Chapter 4.4.6).
...
### 4.11.1 Exercise 1: Add a `@Step` for the Empty State (15 min)

**Objective.** Practice adding a decorated assertion to an existing POM.

**Instructions.**
1. Open `apps/ledger-live-mobile/e2e/page/trade/swapHistory.page.ts` (the POM you wrote in Chapter 4.10 — or a draft of it).
...
### 4.11.2 Exercise 2: A Modal Close Spec (30 min)
...
### 4.11.3 Exercise 3: Android API-Level Matrix Entry (45 min)
...
### 4.11.4 Exercise 4: Multi-Account Userdata + Accounts List Spec (60 min)
...
### 4.11.5 Exercise 5: Feature-Flag Override via Bridge (90 min)
...
**Hints.**
- The bridge `setFeatureFlag` contract is documented in Chapter 4.6. If the payload shape is wrong, the app ignores the message silently — attach a `onerror` logger while iterating.
...
### 4.11.6 Exercise 6: Nightly Swap-Only CI Matrix (90 min)
...
### 4.11.7 Exercise 7: Extend QAA-702 with an Empty-State Path (120 min)
...
### 4.11.8 Challenge: Warm-Simulator Detox Wrapper (120 min, stretch)
